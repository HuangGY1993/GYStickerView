...
/*- (void)touchesBegan:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event {
    ...
    NSLog(@"began:%li",[[event allTouches] allObjects].count);
}

- (void)touchesEnded:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event {
    [super touchesEnded:touches withEvent:event];
    NSLog(@"end:%li",[[event allTouches] allObjects].count);
}

- (void)touchesCancelled:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event {
    NSLog(@"cancel:%li",[[event allTouches] allObjects].count);
}

- (void)touchesEstimatedPropertiesUpdated:(NSSet<UITouch *> *)touches {
    NSLog(@"eses:%li",[touches allObjects].count);
}

- (void)touchesMoved:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event {
    [super touchesMoved:touches withEvent:event];
    NSLog(@"move:%li",[[event allTouches] allObjects].count);
    if ([[event allTouches] allObjects].count > 1) {
        return;
        CGPoint prePoint0 = [[[event allTouches] allObjects][0] previousLocationInView:self];
        CGPoint prePoint1 = [[[event allTouches] allObjects][1] previousLocationInView:self];
        
        CGPoint newPoint0 = [[[event allTouches] allObjects][0] locationInView:self];
        CGPoint newPoint1 = [[[event allTouches] allObjects][1] locationInView:self];
        
        CGFloat preDistance = [self distanceWithStartPoint:prePoint0 endPoint:prePoint1];
        CGFloat newDistance = [self distanceWithStartPoint:newPoint0 endPoint:newPoint1];
        
        CGFloat scale = newDistance / preDistance;
        self.transform = CGAffineTransformScale(self.transform, scale, scale);
        
        CGFloat angle = HGYZDangleBetweenLines(prePoint0, prePoint1, newPoint0, newPoint1);
//        NSLog(@"angle:%f",angle);
        self.transform = CGAffineTransformRotate(self.transform, angle);
//        NSLog(@"Line0:\n%@\n%@",NSStringFromCGPoint(prePoint0),NSStringFromCGPoint(newPoint0));
//        NSLog(@"Line1:\n%@\n%@",NSStringFromCGPoint(prePoint1),NSStringFromCGPoint(newPoint1));
    }
}


CGFloat gy_angleBetweenLines(CGPoint line0Start, CGPoint line0End, CGPoint line1Start, CGPoint line1End) {
    
    CGFloat a = line0End.x - line0Start.x;
    CGFloat b = line0End.y - line0Start.y;
    CGFloat c = line1End.x - line1Start.x;
    CGFloat d = line1End.y - line1Start.y;
    
    CGFloat param = ((a*c) + (b*d)) / ((sqrt(a*a + b*b)) * (sqrt(c*c + d*d)));
    if(param<-1.0)
        param=-1.0;
    else if(param>1.0)
        param=1.0;
    CGFloat rads = acos(param);
    return rads;
    M_PI_4;
}*/
